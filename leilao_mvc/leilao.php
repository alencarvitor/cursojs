...
function atualizarStatus() {
    ...
    .then(data => {
        if (data.success) {
            ...
            mostrarNotificacao('Erro ao atualizar status: ' + data.error, 'error');
        }
    })
...
function selecionarProduto(produtoId) {
    ...
    .then(data => {
        if (data.success) {
            mostrarNotificacao('Produto selecionado com sucesso!');
            setTimeout(() => {
                window.location.reload();
            }, 1000);
        } else {
            mostrarNotificacao('Erro ao selecionar produto: ' + data.error, 'error');
        }
    })
    .catch(err => mostrarNotificacao('Erro: ' + err, 'error'));
}
...
function retornarParaLista(produtoId) {
    if (confirm('Deseja retornar o produto para a lista de espera?')) {
        ...
        .then(data => {
            if (data.success) {
                mostrarNotificacao('Produto retornado para a lista com sucesso!');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
                mostrarNotificacao('Erro ao retornar produto: ' + data.error, 'error');
            }
        })
        .catch(err => mostrarNotificacao('Erro: ' + err, 'error'));
    }
...
function marcarVendido(produtoId) {
    if (confirm('Deseja marcar o produto como vendido?')) {
        ...
        .then(data => {
            if (data.success) {
                mostrarNotificacao('Produto marcado como vendido com sucesso!');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
                mostrarNotificacao('Erro ao marcar como vendido: ' + data.error, 'error');
            }
        })
        .catch(err => mostrarNotificacao('Erro: ' + err, 'error'));
    }
...
function abrirParaLance(produtoId) {
    if (confirm('Deseja abrir o produto para lances?')) {
        ...
        .then(data => {
            if (data.success) {
                mostrarNotificacao('Produto aberto para lances com sucesso!');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
                mostrarNotificacao('Erro ao abrir para lances: ' + data.error, 'error');
            }
        })
        .catch(err => mostrarNotificacao('Erro: ' + err, 'error'));
    }
...
function retornarParaVenda(itemId) {
    if (confirm('Deseja retornar o item para venda?')) {
        ...
        .then(data => {
            if (data.success) {
                mostrarNotificacao('Item retornado para venda com sucesso!');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
                mostrarNotificacao('Erro ao retornar para venda: ' + data.error, 'error');
            }
        })
        .catch(err => mostrarNotificacao('Erro: ' + err, 'error'));
    }
